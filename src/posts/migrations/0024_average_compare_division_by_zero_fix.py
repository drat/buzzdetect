...
AS $$
    ...
    DECLARE post_average_compare_after_three_minute FLOAT;
    BEGIN
        ...
        RAISE LOG 'Average currently is %', new_posteraveragestat.average;
        IF
            new_posteraveragestat.average > 0.0
        THEN
            post_average_compare_after_three_minute := NEW.reposts / new_posteraveragestat.average;
            RAISE LOG 'Post average is %', post_average_compare_after_three_minute;
        ELSE
            post_average_compare_after_three_minute := 0;
            RAISE LOG 'Setting post average to 0';
        END IF;
        RAISE LOG 'Decided of having a compare of % because average is %', post_average_compare_after_three_minute, new_posteraveragestat.average;

        -- We may now reference to this stat as stat_after_three_minute and
        -- update the average at three minute
        UPDATE
            posts_post
        SET
            stat_after_three_minute_id = NEW.id,
            average_compare_after_three_minute = post_average_compare_after_three_minute
        WHERE
            id = NEW.post_id;
        RAISE LOG 'Updated post average_compare_after_three_minute to % for %', post_average_compare_after_three_minute, NEW.post_id;
        RETURN NEW;
    END;
$$ LANGUAGE plpgsql;
'''
